    ...
    if (obufsiz) {
        while (n1 >= 0) {
            ...
            if (n1 == 0) {                                 // le buffer
				...
				if ( header )
				{
					iptr = skip_header_file_utf8(ibuf);
					header = 0;
				}
				else
				{
...
#if UTF8
        	isisuctab_utf8 = loaductb_utf8(mi_gidbnp,p+6);
            if ( !isisuctab_utf8 ) fatal(p);
#else
...
#if UTF8
        	isisactab_utf8 = loadactb_utf8(mi_gidbnp,p+6);
            if ( !isisactab_utf8 ) fatal(p);
            continue;
#else
...
#if UTF8
				fatal("convert is not available for utf8");
#else
...
//#endif /* CICGI */
        ...
        if (strncmp(p,"fullinv",7) == 0) {
            ...
            if (strncmp(q,"/ansi",5) == 0) {
#if UTF8
				fatal("fullinv/ansi is not an utf8 option");
#else
											q+=5;
                memmove(isisuctab,ansiuctab,256/*sizeof(ansiuctab)*/);
                for (isiswctot=0; ansiactab[isiswctot]; isiswctot=isiswctot+1)
    ...
    */
    ...
    if (parmkdbnp) {
        if (parmk1p) {
#if UTF8
			uppercase_utf8(parmk1p);
			if ( strlen(parmk1p) > LE2 ) parmk1p[LE2] = '\0';
#else
        ...
        if (parmk2p) {
#if UTF8
			uppercase_utf8(parmk2p);
			if ( strlen(parmk2p) > LE2 ) parmk2p[LE2] = '\0';
#else
