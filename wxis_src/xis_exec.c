...
#define EXE_REC_SIZE       (rec_maxmfrl) /*1048576L*/ /*4194304L*/ /* defult record size */
#define EXE_BUFF_SIZE      (rec_maxmfrl) /*4194304L*/ /*1025000L*/ /* defult buffer size */
#else
//#define EXE_REC_SIZE       (32L*KBYTE)    /* defult record size */
//#define EXE_BUFF_SIZE      (32L*KBYTE)    /* defult buffer size */
#define EXE_REC_SIZE       (rec_maxmfrl)    /* defult record size */
#define EXE_BUFF_SIZE      (rec_maxmfrl)    /* defult buffer size */
#endif
...
#if UTF8
extern STRUCT_ISISUCTAB_UTF8 isisuctab_utf8_latin[];	// utf-8 upper case table for latin languages
extern STRUCT_ISISACTAB_UTF8 isisactab_utf8_latin[];	// utf-8 alphabetic table for latin languages
#else
                     ...
                     CPL_COMMAND *cmd)       /* current command */
{
   ...
   /* FST generation */
#if !CIFFI
   if (exeVar->buffSize <= EXE_BUFF_SIZE) {
        efc_free(exeVar->buff);
   	exeVar->buffSize = 3*EXE_BUFF_SIZE;
...
#else
   fst_hdrp = exeVar->buff; fst_hdrx = 3*EXE_REC_SIZE-1; fst_hdrt = TRUE;
#endif
...
#else /* CIB71 */
   searchErrorText = b7_run(cib7p,idxHit,db,buff,EXE_REC_SIZE,2L,
#if UTF8
					isisuctab_utf8,
#else
					isisuctab,
#endif // UTF8
		   			"",idxPfx,idxControl,&searchErrorCode);
#endif /* CIB71 */
   b5msg_idx = -1L;
   b5msg_tag = 0;
...
#if UTF8
	isisuctab_utf8 = &(isisuctab_utf8_latin[0]);	// start with standard upper case table
	isisactab_utf8 = &(isisactab_utf8_latin[0]);	// start with standard alphabetic table
#else
