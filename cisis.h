...
#ifndef PACKED2
#if GCC
#if SIXTY_FOUR && _LARGEFILE64_SOURCE
#define PACKED2 __attribute__ ((aligned(2),packed))
#else
#define PACKED2 __attribute__ ((aligned(2)))
#endif
#else /* GCC */
#define PACKED2
#endif /* GCC */
...
typedef struct n1idxe {
    UCHR     key[LE1] PACKED2;          /* gdb .n01 idx key */
    PUNT     punt PACKED2;              /* gdb .n01 idx punt */
} N1IDXE;

typedef struct n2idxe {
    UCHR     key[LE2] PACKED2;          /* gdb .n02 idx key */
    PUNT     punt PACKED2;              /* gdb .n02 idx punt */
} N2IDXE;

typedef struct n1stru {
    PUNT     pos PACKED2;               /* gdb .n01 pos */
    UWORD    ock PACKED2;               /* gdb .n01 ock */
    UWORD    it PACKED2;                /* gdb .n01 it */
    N1IDXE   idx[TWORDN] PACKED2;       /* gdb .n01 idx */
} N1STRU;

typedef struct n2stru {
    PUNT     pos PACKED2;               /* gdb .n02 pos */
    UWORD    ock PACKED2;               /* gdb .n02 ock */
    UWORD    it PACKED2;                /* gdb .n02 it */
    N2IDXE   idx[TWORDN] PACKED2;       /* gdb .n02 idx */
} N2STRU;


typedef struct l1idxe {
    UCHR     key[LE1] PACKED2;          /* gdb .l01 idx key */
#if ISISUNISYS
    UCHR     keyalign[2] PACKED2; /* see ISIS.PAS for UNISYS */
#endif
    INFX     info1 PACKED2;             /* gdb .l01 idx info1 */
    INFO     info2 PACKED2;             /* gdb .l01 idx info2 */
#if LIND
    union {
       INFX    info3 PACKED2;          /* tlc info3 - tot no of docs */
       INFX    info4 PACKED2;          /* tlc info4 - posting type and etc */
    } info3info4;
#endif
} L1IDXE;

typedef struct l2idxe {
    UCHR     key[LE2] PACKED2;          /* gdb .l02 idx key */
#if ISISUNISYS
    UCHR     keyalign[2] PACKED2; /* see ISIS.PAS for UNISYS */
#endif
    INFX     info1 PACKED2;             /* gdb .l02 idx info1 */
    INFO     info2 PACKED2;             /* gdb .l02 idx info2 */
#if LIND
    union {
        INFX info3 PACKED2;             /* tlc info3 - tot no of docs */
        INFX info4 PACKED2;             /* tlc info4 - posting type and etc */
...
typedef struct l1stru {
    PUNT     pos PACKED2;               /* gdb .l01 pos */
    UWORD    ock PACKED2;               /* gdb .l01 ock */
    UWORD    it PACKED2;                /* gdb .l01 it */
    PUNT     ps PACKED2;                /* gdb .l01 ps */
#if LIND
    PUNT     psb PACKED2;               /* tlc gdb .l0x ps backward */
#endif
    L1IDXE   idx[TWORDF] PACKED2;       /* gdb .l01 idx */
} L1STRU;

typedef struct l2stru {
    PUNT     pos PACKED2;               /* gdb .l02 pos */
    UWORD    ock PACKED2;               /* gdb .l02 ock */
    UWORD    it PACKED2;                /* gdb .l02 it */
    PUNT     ps PACKED2;                /* gdb .l02 ps */
#if LIND
    PUNT     psb PACKED2;                       /* tlc gdb .l0x ps backward */
#endif
    L2IDXE   idx[TWORDF] PACKED2;       /* gdb .l02 idx */
} L2STRU;
...
typedef struct lzstru {
    PUNT     pos PACKED2;               /* gdb .l0x pos */
    UWORD    ock PACKED2;               /* gdb .l0x ock */
    UWORD    it PACKED2;                /* gdb .l0x it */
    PUNT     ps PACKED2;                /* gdb .l0x ps */
#if LIND
    PUNT     psb PACKED2;               /* tlc gdb .l0x ps backward */
#endif
    INFX     info1[TWORDF] PACKED2;     /* tlc gdb .l0x idx info1 */
    INFO     info2[TWORDF] PACKED2;     /* tlc gdb .l0x idx info2 */
#if LIND
    INFX     info3[TWORDF] PACKED2;     /* tlc gdb .l0x idx info3 */
#endif
    char     keys[LZKASZ2] PACKED2;     /* compressed keys */
} LZSTRU;
...
char *loadstw();
#if UTF8
extern STRUCT_ISISUCTAB_UTF8 *loaductb_utf8();
extern STRUCT_ISISACTAB_UTF8 *loadactb_utf8();
#else
char *loaductb();
char *loadactb();
#endif	/* UTF8 */
...
/* estructura de la pila de ejecuci?n */
typedef struct pdlstru {
   ...
   LONGX        pdlsize;    /* tama?o actual de lvel */
   LONGX        pdlisiz;    /* tama?o inicial de lvel, also growing step */
   short int   pdlplen;    /* tama?o de los postings */
...
char *b7_run(b7_CIB7 *cib7p, LONGX irec,char *dbnamp,char *buffup, int buffuplen,
            LONGX tell,
#if UTF8
			STRUCT_ISISUCTAB_UTF8 uctab_utf8[],
#else
			unsigned char uctab[],
#endif // UTF8
			char *btch0p,
